...
//重新加载页面  延迟300毫秒等待页面加载
setTimeout(reloading, 300);
function reloading(){
	var isgb2big5=getGb2big5Cookie(JF_cn);
	//alert(isgb2big5);
	if (isgb2big5 == 1) {
		translateBody();
		translateImg();
		$("#gb2big5").attr("title", "点击以简体中文方式浏览");
		$("#gb2big5").html("简体");	
	}
	if (isgb2big5 == 0) {
		translateBody();
		translateImg();
		$("#gb2big5").attr("title", "點擊以繁體中文方式瀏覽");
		$("#gb2big5").html("繁體");	
	}	
}
//页面触发转繁体
function StranBody() {
	var isgb2big5=getGb2big5Cookie(JF_cn);
	//alert("StranBody前"+isgb2big5);
	if(isgb2big5==undefined){
		setGb2big5Cookie(JF_cn, 0);
	}
	/* 如果是繁则则转简 */
	if (isgb2big5 == 1) {
		setGb2big5Cookie(JF_cn, 0);
	...
	/* 如果是简则则转繁 */
	if (isgb2big5 == 0) {
		setGb2big5Cookie(JF_cn, 1);
		translateBody();
		translateImg();
		$("#gb2big5").attr("title", "点击以简体中文方式浏览");
		$("#gb2big5").html("简体");
	}
	//isgb2big5=getGb2big5Cookie(JF_cn);
	//alert("StranBody后"+isgb2big5);
}
...
function translateBody(obj) {
	...
	if (!obj) {
		...
		if (getGb2big5Cookie(JF_cn) == 1) {
			$("title").html(Exchange($("title").html()));
		}
		if (getGb2big5Cookie(JF_cn) == 0) {
			$("title").html(Exchange($("title").html()));
		}
	}
	for (var i = 0; i < objs.length; i++) {
		if(objs[i].title != "" && objs[i].title != null) objs[i].title = Exchange(objs[i].title)
		if (objs[i].alt != "" && objs[i].alt != null) objs[i].alt = Exchange(objs[i].alt);
		if (objs[i].tagName == "INPUT" && objs[i].value != "" && objs[i].type != "text" && objs[i].type != "hidden") objs[i].value = Exchange(objs[i].value);
		if (objs[i].placeholder!= "" && objs[i].placeholder != null)  objs[i].placeholder=Exchange(objs[i].placeholder);
...
function Exchange(text) {
	...
	var isgb2big5=getGb2big5Cookie(JF_cn);
	result= (isgb2big5 == 1) ? toTradition(simple,tradition,text) : toSimple(simple,tradition,text);
	return result;
}
//转成繁体
function toTradition(simple,tradition,text) {
    var result="";
	var transit="";
	for (var i = 0; i < text.length; i++) {
    	transit = simple.indexOf(text[i]);
    	transit =(transit == -1) ? text[i] : tradition[transit];
    	result += transit;
    }
    return result;
}
//转成简体
function toSimple(simple,tradition,text) {
    var result="";
	var transit="";
    for (var i = 0; i < text.length; i++) {
    	transit = tradition.indexOf(text[i]);
    	transit =(transit == -1) ? text[i] : simple[transit];
    	result += transit;
    }
    return result;
}
//设置 cookie
function setGb2big5Cookie(name, value) //cookies设置
{
	//创建一个cookie，有效期为1天
	$.cookie(name, value, {expires: 1,path: '/'});
}
//取 cookie
function getGb2big5Cookie(name) //cookies读取
{
	return $.cookie(name);
}
//接口调用 转繁体
function gb2big5(html) {
	var isgb2big5 = getGb2big5Cookie(JF_cn);
	if (isgb2big5 == 1) {
		function simpPYStr() {
		...
		for (var i = 0; i < html.length; i++) {
			if (simpPYStr().indexOf(html.charAt(i)) != -1)
				str += ftPYStr().charAt(simpPYStr().indexOf(html.charAt(i)));
			else
				str += html.charAt(i);
		}
		return str;
...
function translateImg(){
	var isGb2big5 = getGb2big5Cookie(JF_cn);
	//alert(isGb2big5);
	//页面点击繁体切换按钮时 /* 将有 id img-gb2big5 的背景图片后面加上 _gb2big5 或 去掉 _gb2big5*/
	$.each($("[id=img-gb2big5]"),function(index,value){
		if(isGb2big5==1){
			var imgurl =value.src;
			var arr = imgurl.split(".")
			imgurl=insertStr(imgurl,imgurl.indexOf("."+arr.pop()),"_gb2big5");
			$(this).attr("src",imgurl);
			//console.log(imgurl);
		}
		if(isGb2big5==0){
			var imgurl =value.src;
			var pattern = "_gb2big5";
			imgurl = imgurl.replace(new RegExp(pattern), "");
			
			$(this).attr("src",imgurl);
		}
	});
	//页面点击繁体切换按钮时 /* 将有 class img-gb2big5 的css背景图片后面加上 _gb2big5 或 去掉 _gb2big5*/
	$.each($(".img-gb2big5"),function(index,value){
		if(isGb2big5==1){
			var background =$(this).css("background");
			//console.log(background);		
			var backgroundurl=background.substring(background.indexOf("url(\"")+5,background.indexOf("\")"));	
			var imgurl =backgroundurl;
			var arr = imgurl.split(".");
			imgurl=insertStr(imgurl,imgurl.indexOf("."+arr.pop()),"_gb2big5");
			var backgroundcss = background.substring(0,background.indexOf("url(\"")+5)
								+imgurl
								+background.substring(background.indexOf("\")"));
